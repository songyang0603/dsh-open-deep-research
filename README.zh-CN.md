...
> **项目状态：** `0.1.0-alpha.5` Alpha Public Preview，已使用正式发布的 DSH `0.1.0-rc.8` 完成测试。当前通过 GitHub Releases 分发，尚未发布到 npm。
...
完成后，当前目录中会出现 `dsh-open-deep-research-0.1.0-alpha.5.tgz`。Release 同时提供 `SHA256SUMS`，可用于校验文件。
...
收到任意 HTTP 响应，说明 DNS、TCP 与 TLS 已到达 endpoint。此检查不会验证 Key，也不会完成 MCP 启动。endpoint 无响应时，请使用下方的仅搜索 Profile。
